...
export const SkillCategoryCard = ({ category, index, isCarousel = false }) => {
  ...
  return (
    <motion.div
      ...
      className={`group relative border border-slate-800/80 light:border-slate-200/90 rounded-2xl sm:rounded-3xl bg-slate-900/80 light:bg-white hover:border-cyan-500/50 light:hover:border-indigo-400/50 transition-all duration-300 shadow-lg sm:shadow-xl overflow-hidden flex flex-col justify-between h-[380px] sm:h-[420px] ${
        isCarousel
    ...
    >
      ...
      <div className="p-4 sm:p-6 flex flex-col justify-between h-[calc(100%-0.375rem)]">
        {/* Main Content Area Pinned To Top */}
        <div className="flex-1 overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-800/60 light:border-slate-200">
            <div className="flex items-center gap-2.5 sm:gap-3">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-cyan-500/10 light:bg-indigo-100 flex items-center justify-center text-cyan-400 light:text-indigo-600 shrink-0">
          ...
          {/* Skill Items */}
          <div className="space-y-2 sm:space-y-2.5 overflow-y-auto max-h-[220px] sm:max-h-[250px] pr-1 no-scrollbar">
            {category.skills.map((skill, idx) => (
              <div
                key={idx}
                className="group/item flex items-start gap-2.5 p-1.5 sm:p-2 rounded-xl hover:bg-slate-800/40 light:hover:bg-slate-100/70 transition-all duration-200"
              >
        ...
        <div className="pt-3 border-t border-slate-800/60 light:border-slate-200/60 flex items-center justify-between text-[10px] font-mono text-slate-500 shrink-0">
